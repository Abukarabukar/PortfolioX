...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query(
        value = "select project from Project project left join fetch project.abukarUser",
        countQuery = "select count(project) from Project project"
    )
    Page<Project> findAllWithToOneRelationships(Pageable pageable);

    @Query("select project from Project project left join fetch project.abukarUser")
    List<Project> findAllWithToOneRelationships();

    @Query("select project from Project project left join fetch project.abukarUser where project.id =:id")
