...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "select post from Post post left join fetch post.abukarUser", countQuery = "select count(post) from Post post")
    Page<Post> findAllWithToOneRelationships(Pageable pageable);

    @Query("select post from Post post left join fetch post.abukarUser")
    List<Post> findAllWithToOneRelationships();

    @Query("select post from Post post left join fetch post.abukarUser where post.id =:id")
